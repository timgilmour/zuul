 ...
 * Checks (in order): .env in CWD, then ~/.claude/.env as fallback.
 * First file found wins — later files do not override existing vars.
 */
async function loadEnv(): Promise<void> {
  const candidates = [
    resolve(process.cwd(), '.env'),
    resolve(process.env.HOME || process.env.USERPROFILE || '', '.claude/.env'),
  ];
...
EXAMPLES:
  ...
  generate-image --provider vertex --prompt "A cozy cafe scene" --size 2K --output /tmp/cafe.png

ENVIRONMENT VARIABLES:
  GOOGLE_API_KEY           Required for Google provider (Gemini API / AI Studio)
  GOOGLE_API_KEY_VERTEX    Required for Vertex AI Express Mode (API key auth)
  GOOGLE_CLOUD_PROJECT     Required for full Vertex AI (ADC auth, GCP project ID)
...
VERTEX AI MODES:
  Express Mode (recommended): Set GOOGLE_API_KEY_VERTEX with a Vertex Express key.
    No GCP project setup or ADC needed — just an API key.
  Full Vertex AI: Set GOOGLE_CLOUD_PROJECT and authenticate via ADC:
    gcloud auth application-default login

PROVIDER AUTO-DETECTION:
  If --provider is not specified, the CLI checks for available credentials:
...
MORE INFO:
  Documentation: zuul/SKILL.md
  Render rules:  zuul/core/render-rules.md
`);
  process.exit(0);
}
...
function createGenAIClient(provider: "google" | "vertex"): GoogleGenAI {
  if (provider === "vertex") {
    // Vertex AI Express Mode: API key + vertexai flag
    const apiKey = process.env.GOOGLE_API_KEY_VERTEX;
    if (apiKey) {
      return new GoogleGenAI({ vertexai: true, apiKey });
    }
    // Full Vertex AI: ADC + project/location
    const project = process.env.GOOGLE_CLOUD_PROJECT;
    const location = process.env.GOOGLE_CLOUD_LOCATION || "us-central1";
    if (!project) {
      throw new CLIError("Missing environment variable: GOOGLE_API_KEY_VERTEX (Express Mode) or GOOGLE_CLOUD_PROJECT (full Vertex AI)");
    }
...
async function generateWithNanoBananaPro(
  ...
  if (referenceImage) {
    console.log(`Generating with Nano Banana Pro (Gemini 3 Pro)${providerLabel} at ${size} ${aspectRatio} with reference image...`);
  } else {
    console.log(`Generating with Nano Banana Pro (Gemini 3 Pro)${providerLabel} at ${size} ${aspectRatio}...`);
  }
  ...
  // Vertex AI does not support imageSize — only aspectRatio
...
async function generateWithNanoBanana2(
  ...
  if (referenceImage) {
    console.log(`Generating with Nano Banana 2 (Gemini 3.1 Flash Image)${providerLabel} at ${size} ${aspectRatio}${thinkingLabel}${groundedLabel} with reference image...`);
  } else {
    console.log(`Generating with Nano Banana 2 (Gemini 3.1 Flash Image)${providerLabel} at ${size} ${aspectRatio}${thinkingLabel}${groundedLabel}...`);
  }
  ...
  // Build config — Vertex AI does not support imageSize
  const imageConfig: Record<string, any> = { aspectRatio };
  if (provider !== "vertex") {
    imageConfig.imageSize = size;
  }
...
async function main(): Promise<void> {
  try {
    // Load API keys from .env (CWD) or ~/.claude/.env (fallback)
    await loadEnv();

    const args = parseArgs(process.argv);

    // Determine provider (explicit flag > auto-detect from API keys/credentials)
    let provider: Provider;
    if (args.provider) {
      provider = args.provider;
    } else if (process.env.GOOGLE_API_KEY) {
      provider = "google";
    } else if (process.env.GOOGLE_API_KEY_VERTEX || process.env.GOOGLE_CLOUD_PROJECT) {
      provider = "vertex";
    ...
    const geminiProvider = provider === "vertex" ? "vertex" : "google";
    if (args.seed !== undefined && provider === "openrouter") {
